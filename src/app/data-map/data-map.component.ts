...
export class DataMapComponent implements AfterViewInit {
  ...
  private measurementsUrl =
    'https://api.hcdp.ikewai.org/mesonet/db/measurements?location=hawaii';
    ...
    return new Promise((resolve, reject) => {
      ...
      script.onload = () => {
        ...
        if ((window as any).L?.WindBarb?.icon) {
          ...
          reject(
            'WindBarb script loaded but did not attach correctly to window.L',
          );
        }
      };
  ...
  async fetchStationData(): Promise<void> {
    ...
    try {
      ...
      const variableData: {
        [key: string]: { value: number | null; timestamp: string | null };
      } = {};

      if (varId === 'RF_1_Tot300s') {
        // Sum last 24 hours per station (288 x 5-min intervals)
        const rainMap: {
          [key: string]: { total: number; latestTs: string | null };
        } = {};
        const now = new Date();
        ...
        for (const entry of data) {
          ...
          if (
            !rainMap[sid].latestTs ||
            tsDate > new Date(rainMap[sid].latestTs)
          ) {
      ...
      this.lastLegendMin = minValue;
      this.lastLegendMax = maxValue;
  ...
  private async plotWindBarbs(): Promise<void> {
    ...
    if (
      !(window as any).L?.WindBarb ||
      typeof (window as any).L.WindBarb.icon !== 'function'
    ) {
      return;
    }
    ...
    try {
      ...
      const stationIdMap = new Map(
        activeStations.map((s) => [s.station_id, s]),
      );
      ...
      Object.entries(stationWindMap).forEach(([stationId, wind]) => {
        ...
        if (lat != null && lon != null && WS != null && WDrs != null) {
          ...
          const icon = (window as any).L.WindBarb.icon({
            deg: WDrs,
            speed: speedKMH,
          });
  ...
  async fetchStationDetails(stationId: string): Promise<void> {
    try {
      ...
      for (const variable of variableList) {
        const varId =
          variable === 'RF_1_Tot300s_24H' ? 'RF_1_Tot300s' : variable;
        ...
        try {
          ...
          if (varId === 'RF_1_Tot300s') {
            ...
            updatedDetails['RF_1_Tot300s'] =
              total > 0 ? `${total.toFixed(1)} mm` : '0.0 mm';
      ...
      this.selectedStation.detailsTimestamp = latestTimestamp
  ...
  private addLegend(minValue: number, maxValue: number): void {
    ...
    legend.onAdd = () => {
      ...
      const variableLabel = this.getLegendLabel();
      const { left, right } = this.getLegendMinMaxDisplay(minValue, maxValue);
        ...
        gradientDiv.style.background = `linear-gradient(to right,
            ${
              this.selectedVariable === 'RF_1_Tot300s_24H'
                ...
                : this.selectedVariable === 'SM_1_Avg' ||
                    this.selectedVariable === 'RH_1_Avg'
  ...
  private lastLegendMin: number | null = null;
  private lastLegendMax: number | null = null;
  ...
  convertUnits(): void {
    ...
    if (this.selectedStation.details['Tair_1_Avg']) {
      ...
      this.convertedDetails['Tair_1_Avg'] =
        this.unitSystem === 'standard'
    ...
    if (this.selectedStation.details['Tsoil_1_Avg']) {
      ...
      this.convertedDetails['Tsoil_1_Avg'] =
        this.unitSystem === 'standard'
          ? `${((tempC * 9) / 5 + 32).toFixed(1)}°F`
          : `${tempC.toFixed(1)}°C`;
    }
    ...
    if (this.selectedStation.details['RF_1_Tot300s']) {
      ...
      this.convertedDetails['RF_1_Tot300s'] =
        this.unitSystem === 'standard'
    ...
    if (this.selectedStation.details['WS_1_Avg']) {
      ...
      this.convertedDetails['WS_1_Avg'] =
        this.unitSystem === 'standard'
          ? `${(windMS * 2.23694).toFixed(2)} mph`
          : `${windMS.toFixed(1)} m/s`;
    }
    if (this.selectedStation.details['WDrs_1_Avg']) {
      const windDegrees = parseFloat(
        this.selectedStation.details['WDrs_1_Avg'],
      );
      this.convertedDetails['WDrs_1_Avg'] =
        this.windDirectionToCardinal(windDegrees);
    }
  }
  ...
  private mmToIn(mm: number): number {
    return mm / 25.4;
  }

  private formatLegendValue(value: number): string {
    // Soil moisture legend is special: you show 0–100, not min/max
    if (this.selectedVariable === 'SM_1_Avg') {
      return `${Math.round(value)}`;
    }

    // Temperature
    if (this.selectedVariable === 'Tair_1_Avg' || this.selectedVariable === 'Tsoil_1_Avg') {
      const v = this.unitSystem === 'standard' ? this.toF(value) : value;
      return v.toFixed(1);
    }

    // Rainfall (your map values are mm)
    if (this.selectedVariable === 'RF_1_Tot300s_24H') {
      const v = this.unitSystem === 'standard' ? this.mmToIn(value) : value;
      return this.unitSystem === 'standard' ? v.toFixed(2) : v.toFixed(1);
    }

    // Wind legend is skipped already; RH is percent already; SWin is W/m² already
    return value.toFixed(1);
  }

  private getLegendLabel(): string {
    const isStd = this.unitSystem === 'standard';

    switch (this.selectedVariable) {
      case 'Tair_1_Avg':  return `Air Temperature (${isStd ? '°F' : '°C'})`;
      case 'Tsoil_1_Avg': return `Soil Temperature (${isStd ? '°F' : '°C'})`;
      case 'RF_1_Tot300s_24H': return `24H Rainfall (${isStd ? 'in' : 'mm'})`;
      case 'RH_1_Avg':    return `Relative Humidity (%)`;
      case 'SM_1_Avg':    return `Soil Moisture (%)`;
      case 'SWin_1_Avg':  return `Solar Radiation (W/m²)`;
      default:            return `Unknown Variable`;
    }
  }

  private getLegendMinMaxDisplay(minValue: number, maxValue: number): { left: string; right: string } {
    if (this.selectedVariable === 'SM_1_Avg') {
      return { left: '0', right: '100' };
    }

    // Convert min/max for display only (don’t change your color scaling numbers)
    const left = this.formatLegendValue(minValue);
    const right = this.formatLegendValue(maxValue);

    return { left, right };
  }

  private refreshLegend(): void {
    if (this.selectedVariable === 'wind') return;
    if (this.lastLegendMin == null || this.lastLegendMax == null) return;
    this.addLegend(this.lastLegendMin, this.lastLegendMax);
  }
